...
export function checkStepNumbers(systemNames, stepNumbers) {
  if (!systemNames || !stepNumbers) return false

  const stepsBySystem = {}

  for (let i = 0; i < systemNames.length; i++) {
    const step = stepNumbers[i]
    if (!stepsBySystem[systemNames[i]])
      stepsBySystem[systemNames[i]] = stepNumbers[i]
    else if (step <= stepsBySystem[systemNames[i]]) return false
    stepsBySystem[systemNames[i]] = stepNumbers[i]
  }
