...
export function executeCommands(commands) {
  ...
  if (!commands) return Object.values(memory)

  for (let i = 0; i < commands.length; i++) {
    const [instruction, rest] = commands[i].split(" ")

    const [operand1, operand2] = rest.split(",")

    const valueOperand1 = memory[operand1] || Number(operand1)

    switch (instruction) {
      case "MOV":
        memory[operand2] = valueOperand1 % 256
        break
      case "ADD":
        memory[operand1] = (memory[operand1] + memory[operand2]) % 256
        break
      case "DEC":
        memory[operand1] = memory[operand1] === 0 ? 255 : memory[operand1] - 1
        break
      case "INC":
        memory[operand1] = (memory[operand1] + 1) % 256
        break
      case "JMP":
        if (memory["V00"] !== 0) i = Number(operand1) - 1
      default:
        break
    }
  }
