...
export default function carryGifts(gifts, maxWeight) {
  if (!gifts || !maxWeight) return []

  let currentGiftsGroup = ""
  let currentWeightGroup = 0

  const result = []

  for (let i = 0; i < gifts.length; i++) {
    const currentGift = gifts[i]
    const currentGiftWeight = currentGift.length

    if (currentWeightGroup + currentGiftWeight <= maxWeight) {
      currentGiftsGroup += currentGift + " "
      currentWeightGroup += currentGiftWeight
    } else {
      if (currentGiftsGroup) result.push(currentGiftsGroup.trim())
      currentGiftsGroup = ""
      currentWeightGroup = 0
      if (currentGiftWeight <= maxWeight) i--
    }
  }
  if (currentGiftsGroup) result.push(currentGiftsGroup.trim())
  console.log("ULTIMO: ", result)
  return result
}
